...
function calcule(){
    for(let i=0 ;i<data.length;i++){
        ...
        for(let j=0;j<data[i].length-1;j++){
            ...
            if((count<=3 && count>0 && isIncreasing) || (count>=(-3) && count<0  &&  isDecreasing)){
                message = "Safe"
            }else{
                message = "Unsafe"
            }
            tabledata.push(message)
        }
        tablecases.push(tabledata)
    }
    for(let i = 0;i<tablecases.length;i++){
        if(!tablecases[i].includes("Unsafe")){
            result++
         }
   }
   console.log(result)
